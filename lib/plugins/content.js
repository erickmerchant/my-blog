...
var fn = function(content) {
    ...
    return function(pages, next) {
        ...
        var glob_directory = fn.content_directory + content;
        ...
        glob(glob_directory, {}, function (err, files) {
            ...
            _.each(files, function(file, current) {
                ...
                if(_.has(fn.config.converters, conv)) {

                    parts = path.basename(file, ext).split('.');

                    if(parts.length >= 2) {

                        page.date = moment(parts[0], fn.config.date_formats);
                    ...
                    fs.readFile(file, {encoding: 'utf-8'}, function(err, data){
                        ...
                        fn.config.converters[conv](data.slice(2).join(delim), function(err, content){
...
fn.configure = function(content_directory, config) {

    fn.content_directory = content_directory;
