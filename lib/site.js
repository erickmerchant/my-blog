...
Site.prototype.route = function(route, settings) {

    settings = settings || {};

    this.routes[settings.name || route] = new Route(this, route, settings);
};
...
function Route(app, route, settings) {
...
Route.prototype.build = function() {
    ...
    var glob_directory = route.app.settings.content_directory + route.settings.content;

    if(route.settings.content) {
        ...
        glob(glob_directory, {}, function (err, files) {
            ...
            _.each(files, function(file, current) {
                ...
                if(_.has(route.app.settings.converters, conv)) {

                    parts = path.basename(file, ext).split('.');

                    if(parts.length >= 2) {

                        page.date = moment(parts[0], route.app.settings.date_formats);
                    ...
                    fs.readFile(file, {encoding: 'utf-8'}, function(err, data){
                        ...
                        route.app.settings.converters[conv](data.slice(2).join(delim), function(err, content){

                            page.content = content;

                            page.url = interpolate(route.route, page);
    ...
    Q.when(glob_deferred.promise).then(function(){
        ...
        Q.all(model_promises).then(function(){
            ...
            var middleware = route.settings.middleware ? [route.settings.middleware] : [];
            ...
            Array.prototype.unshift.apply(middleware, route.app.befores);

            Array.prototype.push.apply(middleware, route.app.afters);
            ...
            final = function(pages) {
                ...
                if(route.settings.template) {
                    ...
                    _.each(pages, function(page) {
                        ...
                        route.app.settings.nunjucks.render(route.settings.template, page, function(err, html) {

                            var file = route.app.settings.site_directory + trim.left(page.page.url, '/');
    ...
    return handle_deferred.promise;
};
