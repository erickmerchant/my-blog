...
import {optimizeHTML} from "./app/html.ts";
...
import {NotFoundView} from "./app/templates/not_found.tsx";
// import PostView from "./app/templates/post.ts";
// import HomeView from "./app/templates/home.ts";
import {ResumeView} from "./app/templates/resume.tsx";
import {render, h} from "./jsx.js";
...
if (import.meta.main) {
	...
	await Deno.writeTextFile(
		"./dist/404.html",
		render(<NotFoundView site={site} />)
	);

	const posts = await getPublishedPosts();

	// for (const post of posts) {
	// 	await saveView(`./dist/posts/${post.slug}/index.html`, () =>
	// 		PostView({site, post})
	// 	);
	// }
	...
	for await (const {path} of Fs.expandGlob("./dist/**/*.css")) {
		...
		cssImports.set(path.slice(distDir.length), imports);
	}
	...
	for await (const {path} of Fs.expandGlob("./dist/**/*.js")) {
		...
		jsImports.set(path.slice(distDir.length), imports);
	}

	await moveCacheBusted(Path.join(distDir, "favicon-light.png"));

	await moveCacheBusted(Path.join(distDir, "favicon-dark.png"));
