...
/* The store is the source of truth for the state of our application. Every time dispatch is called the store is called with the previous state (previously returned from calling the store) and any arguments passed to dispatch. It returns a new state. Though an immutable state is not used in this example, using something like immutable.js can be a good idea. Here we just practice disciple and only modify the state inside the store. At the top of this store we set up a default state as a default parameter. We'll use a plain object for our state, but it could be anything (the framework doesn't dictate it being a plain object). The properties of this object are...
  ...
  - completedCount: The count of how many completed todos there are.
...
function store (state = {todos: null, completedCount: 0, current: null, next: 0, currentFilter: 'all'}, action, args) {
  if (state.todos == null) {
    ...
    state.completedCount = data.todos != null ? data.todos.filter(([id, todo]) => todo.completed).length : 0
  ...
  switch (action) {
    ...
    case 'set-current-filter':
      state.currentFilter = typeof args.filter === 'string' ? (args.filter.match(/#?\/?([a-z]*)/)[1] || 'all') : 'all'
      break
    ...
    case 'set-all-todos-completedness':
      ...
      if (args.completed) {
        state.completedCount = state.todos.size
      } else {
    ...
    case 'set-todo-completedness':
      if (state.todos.has(args.id)) {
        ...
        state.completedCount += (args.completed ? 1 : -1)
      }
      break
    ...
    <section class="todoapp">
      ...
      ${ift(state.todos.size > 0, () => {
        return html`<section class="main">
          ...
          <input class="toggle-all" type="checkbox" onclick=${setAllTodosCompletedness(state.todos.size - state.completedCount === 0)} ${ift(state.todos.size - state.completedCount === 0, 'checked')} />
          <ul class="todo-list">
            ${ift(state.todos, ([id, todo]) => {
              /* Here we filter the todos based on the route. We then map our filtered todos to output them. */
              return ift(state.currentFilter === 'all' || (state.currentFilter === 'active' && !todo.completed) || (state.currentFilter === 'completed' && todo.completed), html`<li class="todo ${ift(todo.completed, 'completed')} ${ift(state.current === id, 'editing')}">
                <div class="view">
      ...
      ${ift(state.todos.size > 0, () => {
        return html`<footer class="footer">
          <span class="todo-count">
            <strong>${state.todos.size - state.completedCount}</strong> ${ift(state.todos.size - state.completedCount !== 1, 'items', 'item')} left
          </span>
          <ul class="filters">
