...
function store (state = {
  ...
  switch (action) {
    /* Set state.current to a todo item. */
    case 'set-current':
...
function component ({state, dispatch, next}) {
  ...
  return html`<main>
    <section class="todoapp">
      ...
      ${ift(state.todos.length > 0, () => {
        return html`<section class="main">
          <!-- Another event handler. Also a use of ift to output an attribute conditionally. -->
          <input id="toggle-all" class="toggle-all" type="checkbox" onchange=${setAllTodosCompletedness(state.todos.length - state.completed === 0)} checked="${state.todos.length - state.completed === 0}" autofocus />
          <label for="toggle-all">Mark all as complete</label>
          <ul class="todo-list">
            ${state.todos
              .filter((todo) => state.hash === '#/all' || (state.hash === '#/active' && !todo.completed) || (state.hash === '#/completed' && todo.completed))
              .map((todo) => html`
              <li class="todo ${ift(todo.completed, 'completed')} ${ift(state.current === todo, 'editing')}">
                <div class="view">
                  <input class="toggle" type="checkbox" onchange=${setTodoCompletedness(todo)} checked="${todo.completed}" />
                  <label ondblclick=${setCurrent(todo)}>${todo.title}</label>
                  <button class="destroy" onclick=${removeTodo(todo)}></button>
                </div>
                ${ift(state.current === todo, () => html`<input class="edit" type="text" value="${todo.title}" onkeyup=${exitCurrent(todo)} />`)}
              </li>`)}
          </ul>
        </section>`
      })}
      ${ift(state.todos.length > 0, () => {
        return html`<footer class="footer">
          <span class="todo-count">
            <strong>${state.todos.length - state.completed}</strong> ${ift(state.todos.length - state.completed !== 1, ' items', ' item')} left
          </span>
          <ul class="filters">
  ...
  function setTodoCompletedness (todo) {
    return function (e) {
      e.preventDefault()
