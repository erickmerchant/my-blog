...
function storage (state = {}) {
  ...
  state.todos.forEach((todo, id) => todos.push([id, todo]))

  const next = state.next

  /* Save state.todos and state.next to localStorage. */
  window.localStorage.setItem(storageKey, JSON.stringify({todos, next}))
...
function store (state = {todos: null, completed: 0, current: null, next: 0, hash: '#/all'}, action, args) {
