  ...
  function clearCompletedButton () {
    return state.completed > 0
      ? html`<button
          ...
          onclick=${/* Remove all the completed ones */ () => dispatch('remove-completed-todos')}>
            Clear completed
        </button>`
      : ''
  }
  ...
  function item (todo) {
    ...
    <li class="todo ${todo.completed ? 'completed' : ''} ${state.current === todo ? 'editing' : ''}">
      <div class="view">
        ...
        />
        <label ondblclick=${/* Edit this todo item */ () => dispatch('set-current', {todo})}>${todo.title}</label>
        <button class="destroy" onclick=${/* Remove this todo item */ () => dispatch('remove-todo', {todo})}></button>
      </div>
      ${editInput()}
    </li>`
...
View the live version [here](https://todo.erickmerchant.com).
