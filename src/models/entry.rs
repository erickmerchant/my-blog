		...
		if let Some(paths) = results {
			for path in paths.flatten() {
				if let Ok(content) = fs::read_to_string(&path) {
					let (frontmatter, content) = parse_frontmatter_and_markdown(content);
					let model = Model {
						slug: path
							.file_stem()
...
fn parse_frontmatter_and_markdown(contents: String) -> (frontmatter::Model, String) {
	let parts = contents.splitn(3, "+++");
	let (frontmatter, markdown) = match parts.collect::<Vec<&str>>().as_slice() {
